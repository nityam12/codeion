$(document).ready(function () {
  ...
  var first_name_status,
    signup_email_status,
    signup_password_status,
  ...
  $('#signup_submit').click(function () {
    ...
    // if(first_name_status && signup_email_status && signup_password_status && confirm_password_status)
    // {
  ...
  function validate_signup_password() {
    ...
    if (val_signup_password == '') {
      ...
      val_signup_password.length < 6 == true ||
      val_signup_password.search(/[A-Z]/) == -1 ||
      val_signup_password.search(/[a-z]/) == -1 ||
      val_signup_password.search(/[0-9]/) == -1 ||
      val_signup_password.search(/[ !|@|#|$|%|^|&|*|_ ]/) == -1
    ) {
      message =
        'Requirement: 6 characters, one capital letter, one lowercase letter, atleast one digit and one special character!';
      display_info('#signup_password_error', message);
      $('#signup_password').css('borderColor', 'red');
  ...
  function validate_login_email() {
    ...
    } else if (email_pattern.test(val_login_email) == false) {
      message = 'Your email address is invalid!';
      display_info('#login_email_error', message);
      $('#login_email').css('borderColor', 'red');
      $('#login_email_checked').html('');
  ...
  function display_info(error_div, message) {
    $(error_div)
      .html("<img src='/images/arrow.png'><span style='color:#fff;'>" + message + '</span> ')
      .css({
        background: 'red',
        paddingTop: '10px',
        paddingBottom: '12px',
